...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [uiPermissions] = useState<UIPermissions>({
    // Todas as permissões como true para admin
    sidebar_dashboard: { visible: true, enabled: true },
    sidebar_modules: { visible: true, enabled: true },
    sidebar_clients: { visible: true, enabled: true },
    sidebar_versions: { visible: true, enabled: true },
    sidebar_reports: { visible: true, enabled: true },
    sidebar_audit: { visible: true, enabled: true },
    sidebar_users: { visible: true, enabled: true },
    button_create_module: { visible: true, enabled: true },
    button_edit_module: { visible: true, enabled: true },
    button_delete_module: { visible: true, enabled: true },
    button_create_client: { visible: true, enabled: true },
    button_edit_client: { visible: true, enabled: true },
    button_delete_client: { visible: true, enabled: true },
    button_create_version: { visible: true, enabled: true },
    button_edit_version: { visible: true, enabled: true },
    button_delete_version: { visible: true, enabled: true },
    section_dashboard_metrics: { visible: true, enabled: true },
    section_dashboard_charts: { visible: true, enabled: true },
    feature_export_data: { visible: true, enabled: true },
    feature_bulk_actions: { visible: true, enabled: true },
    feature_advanced_filters: { visible: true, enabled: true }
  })
  ...
  const isUIVisible = (elementKey: string) => {
    return uiPermissions[elementKey]?.visible ?? true
  }
  const isUIEnabled = (elementKey: string) => {
    return uiPermissions[elementKey]?.enabled ?? true
  }
  const checkUIPermissions = (elementKeys: string[], type: 'visible' | 'enabled' = 'visible') => {
    const result: Record<string, boolean> = {}
    elementKeys.forEach(key => {
      result[key] = uiPermissions[key]?.[type] ?? true
    })
    return result
  }
...
export function useUIPermissions() {
  ...
  return {
    ...
    isVisible: auth.isUIVisible,  // Mapear para isVisible
    isEnabled: auth.isUIEnabled,  // Mapear para isEnabled
